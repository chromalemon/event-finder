...
class BaseEventForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if getattr(self.instance, "pk", None):
            going_count = EventAttendee.objects.filter(
                event=self.instance, status="going"
            ).count()
    ...
    def save(self, commit=True, host=None):
        """
        Create/replace location, assign host if new event,
        and replace categories before saving to DB.
        """
        ...
        if addr and lat is not None and long is not None:
            try:
                existing_loc = Location.objects.filter(
                    formatted_address=addr, lat=lat, long=long
                ).first()
            ...
            try:
                online_loc = Location.objects.filter(
        ...
        if commit:
            ...
            if host:
                EventAttendee.objects.get_or_create(
                    event=event, user=host, defaults={"status": "going"}
                )
