...
class ChatTests(TestCase):
    def test_chat_page_requires_login_as_host_or_attendee(self):
        ...
        random_user = User.objects.create_user(username="random_user", email="random@example.com", password="pass12345")
        self.client.login(username="random_user", password="pass12345")
        response = self.client.get(url)
        self.assertContains(response, "Access Restricted")

        EventAttendee.objects.create(event=event, user=random_user, status="going")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
